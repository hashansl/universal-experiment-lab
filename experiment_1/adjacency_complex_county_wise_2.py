...
def process_state(state, selected_variables, selected_variables_with_censusinfo,df,mort_df):
    ...
    for variable_name in selected_variables:
        ...
        sum_mor = mort_df[mort_df['ST_ABB'] == state][variable_name].sum()

        new_row = pd.DataFrame([{ 'State': state, 'Total_life_span': TL, 'Sum_Mor': sum_mor, 'Variable': variable_name, 'county_count': len(mort_df)}])
...
if __name__ == "__main__":
    ...
    selected_variables = ['MOR_14','MOR_15','MOR_16','MOR_17','MOR_18','MOR_19','MOR_20']
    ...
    # TL = process_state(state, selected_variables, selected_variables_with_censusinfo,df,mortality_df)
    ...
    tl_df = pd.DataFrame(columns=['State', 'Total_life_span', 'Sum_Mor', 'Variable','county_count'])
    ...
    for state in tqdm(states, desc="Processing states"):
        ...
        tl_df = process_state(state, selected_variables, selected_variables_with_censusinfo,tl_df,mortality_df)

    # sace the df to csv

    tl_df.to_csv('/home/h6x/git_projects/universal-experiment-lab/experiment_1/data/total_life_span_mortality_each_year.csv', index=False)
